...
data = {
    'Zon Taburan Hujan': ['Tinggi 1'] * 25,
    'Kelas Tanah': ['Rata'] * 25,
    'Topografi': ['1'] * 25,
    'Tahun Tuai': list(range(1, 26)),
    'Potensi Hasil': [
...
def create_streamlit_app():
    ...
    if not filtered_df.empty:
        ...
        # Visualization
        st.subheader('Grafik Potensi Hasil Berdasarkan Tahun')
        chart_data = df.groupby('Tahun Tuai')['Potensi Hasil'].mean().reset_index()
        st.line_chart(chart_data.set_index('Tahun Tuai'))
